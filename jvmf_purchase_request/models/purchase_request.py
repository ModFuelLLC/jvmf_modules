...
class PurchaseRequest(models.Model):
    ...
    def _track_subtype(self, init_values):
        for rec in self:
            if 'state' in init_values and rec.state == 'to_approve':
                return 'jvmf_purchase_request.mt_request_to_approve'
            elif 'state' in init_values and rec.state == 'approved':
                return 'jvmf_purchase_request.mt_request_approved'
            elif 'state' in init_values and rec.state == 'rejected':
                return 'jvmf_purchase_request.mt_request_rejected'
    ...
    requested_by = fields.Many2one('res.users', 'Requested by', track_visibility='onchange', domain=[('partner_id.purchase_request_allowed','=',True)], default=_get_default_requested_by)
    partner_id = fields.Many2one('res.partner', 'Related Partner', related='requested_by.partner_id')
    ...
    def action_view_po(self):
        ...
        action = self.env.ref('jvmf_purchase_request.pr_po_tree').read()[0]
        if len(pos) > 1:
