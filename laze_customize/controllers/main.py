...
class Web_Editor(Web_Editor):
    ...
    def save_scss(self, url, bundle_xmlid, content):
        ...
        if url != '/laze_customize/static/src/scss/options/colors/color_picker.scss':
        ...
        custom_url = '/laze_customize/static/src/scss/options/colors/color_picker'+str(request.website.id)+'.scss'
...
class Website(Website):
    ...
    def theme_customize(self, enable, disable, get_bundle=True):
        ...
        if enable:
            for i in enable:
                if i.startswith('laze_customize.header_layout_'):
                     if  not i.endswith('_css'):
                         templ = i+'_css'
                         views = request.env['ir.ui.view'].search([('key','ilike','laze_customize.header_layout_')])
                         for v in views:
                            if v.key.endswith('_css'):
                                set_active([v.key], False)
                         set_active([templ], True)

                if i.startswith('laze_customize.footer_layout_'):
                     if  not i.endswith('_css'):
                         templ = i+'_css'
                         views = request.env['ir.ui.view'].search([('key','ilike','laze_customize.footer_layout_')])
                         for v in views:
                            if v.key.endswith('_css'):
                                set_active([v.key], False)
                         set_active([templ], True)
...
class WebsiteSale(WebsiteSale):
    ...
    def variant_change_images(self, product):
        ...
        if data:
            return http.request.render('laze_customize.image_box_product', {
                    'variant': data,
    ...
    def update_cart_popup(self):
        order = request.website.sale_get_order()
        return request.render("laze_customize.product_cart", {'website':request.website})
    ...
    def cart(self, access_token=None, revive='', **post):
        ...
        if post.get('type') == 'cart_lines_popup':
            return request.render('laze_customize.cart_lines_popup_content', res.qcontext)
        else:
            return res
    ...
    def check_magnifier_status(self, **kw):
        # status = False
        view_rec = request.env['ir.ui.view'].search([('key','=like','laze_customize.product_picture_magnify')]) #('id','=',1307),('key','=like','laze_customize.product_picture_magnify')
