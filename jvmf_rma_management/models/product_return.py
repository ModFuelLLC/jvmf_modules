...
class ProductReturn(models.Model):
    ...
    #set partner_id based off of product return type
    @api.multi
    def _set_partner(self):
        for rma in self:
            if rma.product_return_type == 'outgoing':
                rma.partner_id = rma.vendor_id
                rma.customer_id = 0
            elif rma.product_return_type == 'incoming':
                rma.partner_id = rma.customer_id
                rma.vendor_id = 0
    ...
    product_return_type = fields.Selection([('incoming', 'Customer'), ('outgoing', 'Vendor')], string="Return Type", required=True, readonly=True, states={'draft': [('readonly', False)]},
    ...
    vendor_id = fields.Many2one('res.partner', string='Vendor', domain=[('supplier', '=', True)], readonly=True, states={'draft': [('readonly', False)]})
    customer_id = fields.Many2one('res.partner', string='Customer', domain=[('customer', '=', True)], readonly=True, states={'draft': [('readonly', False)]})
    ...
    customer_source_id = fields.Many2one('stock.location', string='Customer Source Location', domain=[('active', '=', True), ('usage', '=', 'customer')], readonly=True, states={'draft': [('readonly', False)]})
    vendor_source_id = fields.Many2one('stock.location', string='Vendor Source Location',  domain=[('active', '=', True), ('usage', '=', 'internal')], readonly=True, states={'draft': [('readonly', False)]})

    customer_destination_id = fields.Many2one('stock.location', string='Customer Destination Location',  domain=[('active', '=', True), ('usage', '=', 'internal')], readonly=True, states={'draft': [('readonly', False)]})
    vendor_destination_id = fields.Many2one('stock.location', string='Vendor Destination Location', domain=[('active', '=', True), ('usage', '=', 'supplier')], default=lambda self: self.env['stock.location'].search([('active', '=', True), ('usage', '=', 'supplier')]), readonly=True, states={'draft': [('readonly', False)]})
    ...
    @api.onchange('customer_source_id', 'vendor_source_id')
    def _compute_source(self):
        for rma in self:
            if rma.product_return_type == 'incoming':
                rma.source_location_id = rma.customer_source_id
            elif rma.product_return_type == 'outgoing':
                rma.source_location_id = rma.vendor_source_id

    @api.onchange('customer_destination_id', 'vendor_destination_id')
    def _compute_destination(self):
        for rma in self:
            if rma.product_return_type == 'incoming':
                rma.destination_location_id = rma.customer_destination_id
            elif rma.product_return_type == 'outgoing':
                rma.destination_location_id = rma.vendor_destination_id
    ...
    def write(self, vals):
        if self.product_return_type == 'incoming':
            ...
            if 'customer_source_id' in vals:
                vals['source_location_id'] = vals['customer_source_id']
            else:
                vals['source_location_id'] = self.customer_source_id.id
            if 'customer_destination_id' in vals:
                vals['destination_location_id'] = vals['customer_destination_id']
            else:
                vals['destination_location_id'] = self.customer_destination_id.id
            vals['vendor_id'] = 0
            vals['vendor_source_id'] = 0
            vals['vendor_destination_id'] = 0
        elif self.product_return_type == 'outgoing':
            if 'vendor_id' in vals:
                vals['partner_id'] = vals['vendor_id']
            else:
                vals['partner_id'] = self.vendor_id.id
            if 'vendor_source_id' in vals:
                vals['source_location_id'] = vals['vendor_source_id']
            else:
                vals['source_location_id'] = self.vendor_source_id.id
            if 'vendor_destination_id' in vals:
                vals['destination_location_id'] = vals['vendor_destination_id']
            else:
                vals['destination_location_id'] = self.vendor_destination_id.id
            vals['customer_id'] = 0
            vals['customer_source_id'] = 0
            vals['customer_destination_id'] = 0
    ...
    def create(self,vals):
        if vals['product_return_type'] == 'incoming':
            vals['partner_id'] = vals['customer_id']
            vals['source_location_id'] = vals['customer_source_id']
            vals['destination_location_id'] = vals['customer_destination_id']
            vals['vendor_id'] = 0
            vals['vendor_source_id'] = 0
            vals['vendor_destination_id'] = 0
        elif vals['product_return_type'] == 'outgoing':
            vals['partner_id'] = vals['vendor_id']
            vals['source_location_id'] = vals['vendor_source_id']
            vals['destination_location_id'] = vals['vendor_destination_id']
            vals['customer_id'] = 0
        ...
        if rma.product_return_type == 'incoming' and rma.company_id.rma_followup_contact and rma.company_id.rma_followup_contact.partner_id not in rma.message_partner_ids:
            rma.message_subscribe(partner_ids=[rma.company_id.rma_followup_contact.partner_id.id])
        return rma
...
class ProductReturnLine(models.Model):
    ...
    @api.depends('quantity', 'last_price_unit','price_unit')
    def _compute_amount(self):
        ...
        for line in self:
            ...
            line.update({
                'price_total': price * line.quantity
            })
    ...
    def _check_price_unit(self):
        for line in self:
            ...
            if line.return_id.product_return_type == 'incoming':
                line.price_unit = line.return_id.customer_id.property_product_pricelist.get_product_price(line.product_id,1,line.return_id.customer_id)
    ...
    def _onchange_option(self):
        ...
        if self.product_id:
            ...
            if self.return_id.product_return_type == 'incoming':
                self.price_unit = self.return_id.customer_id.property_product_pricelist.get_product_price(self.product_id,1,self.return_id.customer_id)
